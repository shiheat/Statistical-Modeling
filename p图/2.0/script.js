document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateFlowVisualization(timestamp) {
        ...
        // Use the stored defs element if available, otherwise query it
        const currentDefs = flowSvg.querySelector('defs') || svgDefs.cloneNode(true);
        flowSvg.appendChild(currentDefs);

        // Ensure coordinates are calculated before drawing
        if (Object.keys(locationCoords).length === 0) {
             console.warn("Coordinates not ready for drawing.");
             updateLocationCoordinates(); // Try calculating again
             if (Object.keys(locationCoords).length === 0) return; // Exit if still not ready
        }
        ...
        predictedFlows.forEach(flow => {
            ...
            if (fromCoord && toCoord) {
                ...
                // Adjust stroke width mapping for better visual range
                const strokeWidth = Math.max(1.5, Math.min(1 + flow.volume * 1.0, 6)); // Range 1.5px to 6px
