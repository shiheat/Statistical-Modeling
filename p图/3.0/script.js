document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 1. 地点数据 (名称和百分比坐标) ---
    const locations = [
        // REMOVED single quotes from names
...
// ... (Keep everything the same until updateFlowVisualization function)

    // --- 8. 更新流量可视化 ---
    function updateFlowVisualization(timestamp) {
        const predictedFlows = getPredictedFlow(timestamp);
        flowSvg.innerHTML = ''; // Clear previous lines

        // Re-append the defs - IMPORTANT!
        const currentDefs = flowSvg.querySelector('defs') || svgDefs.cloneNode(true);
         if (!flowSvg.contains(currentDefs)) { // Append only if not already there
             flowSvg.appendChild(currentDefs);
         }

        if (Object.keys(locationCoords).length === 0) {
            console.warn("Coordinates not ready for drawing.");
            updateLocationCoordinates();
            if (Object.keys(locationCoords).length === 0) return;
        }
        ...
        predictedFlows.forEach(flow => {
            ...
            if (fromCoord && toCoord) {
                ...
                // Adjust stroke width for the tech style (thinner lines)
                // Map volume (e.g., 0-10) to stroke width (e.g., 1px - 3px)
                const baseWidth = 1;
                const maxWidth = 3;
                const volumeScale = 0.2; // Adjust this multiplier based on typical flow.volume values
                const strokeWidth = Math.max(baseWidth, Math.min(baseWidth + flow.volume * volumeScale, maxWidth));
                line.setAttribute('stroke-width', strokeWidth);

                // Only add arrowhead if line has length (prevents dot at origin)
                ...
                flowSvg.appendChild(line);
                 ...
                 // console.warn(`Coordinates missing for flow: ${flow.from} -> ${flow.to}`);
            }
        });
    }
